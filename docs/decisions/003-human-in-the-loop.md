...
1. **Human-in-the-loop:** Kevin approves every deploy. Confirmation requested with a summary of changes, risk level, checks, rollback plan, and cost when meaningful.
...
- The deploy approval UX (Phase 4) — clear summary, risk level, checks, rollback plan, and confirm/deny command — takes seconds to use. The friction is negligible.
...
- Phase 4 (Discord approval UX) is critical path. Without clean deploy summaries, the approval step becomes a bottleneck.
